...
│     예: "telegram:1613476146", "slack:U07ABC"     │
│                                                  │
...
│                                                  │
│  3. 격리 5층 적용                                   │
│     [도구]   --allowedTools / --disallowedTools   │
│     [디렉토리] --add-dir (from_projects 동적 로드)  │
│     [cwd]   ~/.jarvis/sandboxes/{user_id}/       │
│             (빈 디렉토리 → 홈 탐색 차단)            │
│     [메모리] env JARVIS_USER_ID 주입                │
│             → MCP 서버가 자동으로 user별 분리        │
│     [세션]  --session-id <UUID>                   │
│             (user_id별 영속 매핑 → 이전 대화 복원)   │
│                                                  │
│  4. claude 자식 프로세스 spawn                     │
│     ├─ 위 인자/환경/cwd로 격리된 채 실행           │
│     └─ stdout에 응답 텍스트 반환                    │
│                                                  │
│  5. 응답을 채널 sendMessage로 전송                  │
└──────────────────────────────────────────────────┘
             │
             ▼
          팀원 앱에 응답 도착
...
데몬은 계속 켜져 있고, 요청마다 `claude` 자식 프로세스를 짧게 스폰했다가 종료합니다.
같은 유저의 다음 메시지는 같은 `--session-id`로 호출되어 claude가 이전 대화를 자동 복원하므로 컨텍스트 유지.
...
팀원: /dev UserService에 이메일 검증 추가
    │
    ▼
Jarvis: "어떤 프로젝트에서 작업할까요? 1. my-backend"
    │
팀원: 1
    │
...
| channels.yml | **라우팅 정책(auto_pair 등)은 즉시** (mtime 캐시), 채널 on/off는 데몬 재시작 필요 |
...
│   - 6자리 코드 + 24시간 만료              │
│   - 채널 정책이 auto_pair인 경우(Slack 멘션 │
│     등) 제한 프로필(macho 등)로 즉시 등록 │
└─────────────┬────────────────────────────┘
              ▼
┌──────────────────────────────────────────┐
...
`owner`는 고정이고, 나머지는 저장소에 **예시로 포함된 기본 프로필**입니다. `jarvis edit-profile`로 자유롭게 수정하세요.

| 프로필 | 도구 제한 | 디렉토리 제한 | 시간 제한 | 모델 | 세션 TTL |
|--------|---------|-------------|---------|------|---------|
| **owner** (고정) | 없음 (skip_permissions) | 없음 — 로컬 전체(`/`) | 10분 | opus | 무제한 |
| `developer` (예시) | 읽기/쓰기/git 작업/테스트 | 프로젝트만 | 5분 | opus | 72h |
| `reviewer` (예시) | 읽기 + git 조회만 | 프로젝트만 | 2분 | opus | 24h |
| `observer` (예시) | 읽기/검색만 | 프로젝트만 | 1분 | haiku | 6h |
| `macho` (예시) | 외부 검색/API (로컬 접근 X) | 설정으로 확장 가능 | 3분 | sonnet | 6h |

`owner` 프로필의 경우 `skip_permissions: true` 때문에 모든 제한을 우회합니다. cwd가 `$HOME`으로 시작하고 `--add-dir /`로 로컬 파일시스템 전체(시스템 경로 포함)에 접근 가능합니다. **Owner 본인 전용**이며 다른 팀원에게 부여하지 마세요.

**모델 차등**: 프로필별 `claude.model` 필드로 지정. 복잡 작업 위주 프로필은 `opus`, 밸런스는 `sonnet`, 단발 질문은 `haiku`로 비용/응답시간/정확도를 조정합니다.

**세션 TTL**: 마지막 메시지 이후 이 시간만큼 무활동이면 다음 메시지 수신 시 jsonl 삭제 + 새 UUID 자동 발급. `/clear`로 수동 초기화, `/compact`로 요약 후 이어가기.
