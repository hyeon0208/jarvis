...
매 1분마다 (cron runner):
  ...
  - cron은 stateless 실행 — 매번 새 UUID로 --session-id (이전 대화 맥락에 의존 X)
    메모리 격리는 JARVIS_USER_ID 환경변수로 별도 보장
...
[5] 대화 컨텍스트 (단기)
    scope 결정:
      ...
      · 그 외(DM, Telegram 등)      → user_id
        (개인 세션 — 기존 동작)
      · cron 트리거                 → 매 실행마다 새 UUID (resume 없음, stateless)
    UUID 저장:
...
### 토큰 만료 / revoke (봇이 응답 안 함)

- **증상**: `jarvis logs`에 `[telegram] polling 연속 5회 실패` 또는 Slack `not_authed`, Discord `401` 같은 에러 반복
- **원인**: 봇 토큰이 만료·revoke됨 (Telegram BotFather `/revoke`, Slack `Reinstall App`, Discord `Reset Token` 등)
- **조치**:
  1. `jarvis doctor`로 FAIL 라인 확인 (`Telegram 토큰`, `Slack Bot 토큰` 등)
  2. 플랫폼에서 **새 토큰 발급**
  3. `jarvis channel token <채널명> <새 토큰>` 또는 `.env` 직접 편집
  4. `jarvis restart`로 데몬 재시작 (토큰은 프로세스 env 기반이라 재시작 필요)

### Claude 구독 토큰 매일 만료 (cron이 매일 새벽 실패)

- **증상**: cron 응답이 매일 같은 시각부터 `claude 호출 실패 (코드: 1) — Not logged in · Please run /login` 으로 떨어짐. 수동으로 `claude` 한 번 실행하면 다음 날까지 정상.
- **원인**: Claude Pro/Max 구독의 OAuth refresh token TTL이 약 24시간이라 매일 만료됩니다. GUI 셸에서 `claude` 실행 시 자동 재로그인되지만, 데몬과 cron은 무인 환경이라 `/login`을 띄울 수 없습니다.
- **조치 (구독 그대로 + 만료 없음)** — Claude Code가 공식 제공하는 long-lived token을 발급해 환경변수로 주입합니다.

  ```bash
  # 1) long-lived token 발급 (대화형 — 브라우저에서 승인 후 토큰 출력)
  claude setup-token

  # 2) 출력된 토큰을 ~/jarvis/.env에 추가 (gitignore 처리되어 안전)
  echo 'CLAUDE_CODE_OAUTH_TOKEN=<출력된-토큰>' >> ~/jarvis/.env

  # 3) 데몬 재시작 — .env가 자동 로드되어 spawn되는 claude 자식에 환경변수로 상속됨
  jarvis restart
  ```

- **원리**: 데몬은 시작 시 `~/jarvis/.env`를 `process.env`에 로드하고 (`daemon.ts:loadEnv`), claude 자식 spawn 시 `...process.env`로 그대로 상속합니다. claude CLI가 `CLAUDE_CODE_OAUTH_TOKEN`을 발견하면 keychain의 만료된 OAuth 대신 이 long-lived token을 사용합니다.
- **검증**: 다음 cron 실행 시각 또는 강제 호출 후 `jarvis logs`에 `Not logged in` 패턴이 사라지는지 확인.
- **대안** — long-lived token 발급이 어렵다면 Anthropic Console에서 `ANTHROPIC_API_KEY` 발급 후 같은 방식으로 `.env`에 추가. 단 구독 정액제가 아니라 사용량별 과금으로 청구됩니다.

### 페어링 코드 만료 (24시간)

- **증상**: 팀원이 `jarvis pair approve A1B2C3 ...` 실행 시 "만료된 코드" 에러
- **원인**: 페어링 요청은 24시간 후 자동 만료
- **조치**: 팀원이 봇에 다시 메시지 전송 → 새 코드 발급 → 즉시 승인

### SQLite 메모리 DB 손상 (jarvis_memory_* 호출 시 에러)

- **증상**: `SQLITE_CORRUPT` 또는 `malformed database` 류 에러. `jarvis doctor`의 메모리 DB 크기 체크에서 FAIL
- **원인**: 디스크 가득/정전/강제 종료로 인한 WAL 손상
- **조치**:
  1. 데몬 정지: `jarvis stop`
  2. 백업: `cp ~/.jarvis/data/memory.db ~/.jarvis/data/memory.db.broken`
  3. 재생성: `rm ~/.jarvis/data/memory.db ~/.jarvis/data/memory.db-*`
  4. 데몬 재시작: `jarvis start` (다음 MCP 호출 시 빈 DB 자동 생성)
  5. 과거 세션 파일은 `~/.claude/projects/**/{UUID}.jsonl`에 그대로 남아 있으므로 맥락은 유지됨. 선언적 메모리/스킬 인덱스만 재구성 필요 (`/jarvis sync`)

### thread-sessions.json / users/*.json 손상

- **증상**: `jarvis logs`에 `JSON.parse 실패` 또는 특정 유저가 매번 신규 세션으로 시작
- **원인**: 과거 비원자적 write로 반쪽 JSON이 남음 (atomic write 도입 전 기록)
- **조치**: 해당 파일 삭제 → 데몬이 다음 요청부터 새 상태로 초기화 (세션 UUID만 재발급, 장기 메모리는 영향 없음)
