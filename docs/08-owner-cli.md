...
> | 개념 | 의미 | 접근 경로 |
> |------|------|----------|
> | **Owner (호스트)** | Jarvis를 호스팅하는 본인 | 로컬 터미널 (jarvis 쉘 명령) |
> | **`owner` 프로필** | Owner 본인이 외부 채널에서 자신에게 부여하는 최고 권한 | Telegram/Discord/Slack DM |
>
> `owner` 프로필은 이름 그대로 **Owner 본인 전용**입니다. 다른 팀원에게는 절대 부여하지 마세요.
> `skip_permissions: true`가 설정되어 있어 `jarvis` 쉘 명령까지 실행 가능합니다.
...
### owner 프로필 (외부 채널 최고 권한, Owner 본인 전용)

Owner 본인이 Telegram 등 외부 채널로 자신의 계정에 부여하는 프로필입니다. `skip_permissions: true`라서 **Claude Code의 도구 허용 체크를 우회**하며, 이론상 `jarvis` 쉘 명령까지 실행 가능합니다.
...
| **명령 실행** | 모든 Bash 명령 (이론상 `jarvis pair ...` 포함) |

> ⚠ 이 권한이 무거워 보여도 **Owner 본인의 외부 접속 계정에만** 부여하기 때문에 현실적 위험은 낮습니다.
> 절대 다른 팀원에게 `owner` 프로필을 부여하지 마세요. 팀원 최고 권한은 `developer`까지입니다.

### 다른 프로필 (developer / reviewer / observer)

팀원용 프로필입니다. `--allowedTools` 허용 목록으로 권한이 정확히 제한됩니다.

다음은 저장소에 **예시로 포함된 기본 프로필**입니다. `jarvis edit-profile`로 자유롭게 수정 가능합니다.

| 프로필 | AI 대화 | 크론잡 | /dev | 코드 수정 | Git | 명령 실행 |
|--------|---------|--------|------|----------|-----|----------|
| owner (고정) | ✓ | ✓ | ✓ | ✓ 전체 | ✓ 전체 | ✓ 전체 |
| developer (예시) | ✓ | ✓ | ✓ | ✓ 프로젝트만 | ✓ add/commit/push | ✓ test/build |
| reviewer (예시) | ✓ | ✓ | ✗ | ✗ | ✓ 읽기만 | ✗ |
| observer (예시) | ✓ | ✓ | ✗ | ✗ | ✗ | ✗ |

프로필별 상세 커스터마이징: [04-team-members.md](04-team-members.md) 참고.

### 한 줄 요약

```
Owner (호스트) = 로컬 터미널의 본인.            Jarvis 자체를 관리 (jarvis CLI).
owner 프로필   = Owner의 외부 채널 계정.         Claude를 무제한 활용.
나머지 프로필   = 팀 구성에 맞게 자유롭게 정의.    커스터마이징 필수.
```
...
[ERROR]   ✗ project[backend].allowed_profiles: 존재하지 않는 프로필 참조: owner
